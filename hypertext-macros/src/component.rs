...
use quote::quote;
use syn::{
    FnArg, Ident, ItemFn, LitBool, Pat, PatType, Path, Token, Type, Visibility, parse::Parse,
    parse_quote,
};
...
impl Parse for BuilderArg {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        if name != "builder" {
            return Err(syn::Error::new(name.span(), "unknown argument"));
        }
        ...
        let builder = if input.peek(LitBool) {
            ...
            if lit_bool.value {
                return Err(syn::Error::new(lit_bool.span(), "unexpected `true`"));
            }
            Self::False
        } else {
...
pub fn generate(args: ComponentArgs, mut fn_item: ItemFn) -> syn::Result<TokenStream> {
