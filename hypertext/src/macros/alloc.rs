...
/// Alias for [`rsx!`] using RSX syntax, returning a [`Lazy`](crate::Lazy).
///
/// This exists because Dioxus CLI (`dx serve --hotpatch`) intercepts `rsx!`
/// invocations by name, fails to parse hypertext's syntax, and silently
/// swallows the change. Using `html!` avoids that collision.
///
...
/// Alias for [`rsx_static!`] that avoids the Dioxus CLI `rsx!` name collision.
///
/// See [`html!`] for details.
#[cfg_attr(docsrs, doc(cfg(feature = "alloc")))]
